...
import { exportData, importData, decodeSyncData, getSyncParam, clearSyncParam } from './utils/sync'
...
--- INTERNSHIP STRATEGY ---
Tier 1 (urgent): CERN Short Term (1587 CHF/month, open year-round), CERN Technical (3486 CHF/month, deadline June 30), Polytechnique Montréal email Dr. Foundjem, CNES, Airbus, ESA.`
}
...
export default function App() {
  ...
  // Load from localStorage + check for QR sync param
  useEffect(() => {
    // Check for QR sync first
    const syncParam = getSyncParam()
    if (syncParam) {
      try {
        const data = decodeSyncData(syncParam)
        applyImportedData(data)
        setImportBanner(true)
        setTimeout(() => setImportBanner(false), 4000)
        clearSyncParam()
        return // don't overwrite with localStorage
      } catch (e) {
  ...
  const applyImportedData = (data) => {
    if (data.profile) {
      const merged = { ...STEPHANE, ...data.profile }
      setProfile(merged)
      localStorage.setItem('cm_profile', JSON.stringify(merged))
    }
  ...
  const handleExport = () => exportData({ profile, opps, learning, programme })
  ...
  const switchToNewPerson = () => {
    if (!confirm('Start fresh? This clears all local data.')) return
    localStorage.clear()
    setProfile({ name:'',email:'',role:'',education:'',skills:'',projects:'',goals:'',targets:'',docContext:'' })
    setOpps([]); setLearning(DEFAULT_LEARNING); setProgramme({}); setApiKeys({ groq:'',claude:'' })
    setTab('profile')
  }

  const handleAnalyze = (opp) => {
    setChatInit(`Analyze this opportunity for me — am I a good fit and what should I emphasize? ${opp.role} at ${opp.company}${opp.notes ? '. Details: ' + opp.notes : ''}`)
    setTab('chat')
  }
  const handleCoverLetter = (opp) => {
    setChatInit(`Write a tailored cover letter for: ${opp.role} at ${opp.company}. ${opp.notes ? 'Details: ' + opp.notes : ''} Make it genuine and specific.`)
    setTab('chat')
  }
  ...
  const renderContent = () => {
    switch (tab) {
      ...
      case 'settings': return <Settings apiKeys={apiKeys} onSave={saveKeys} onSwitchPerson={switchToNewPerson} onExport={handleExport} onImport={handleImport} profile={profile} opps={opps} learning={learning} programme={programme} />
      default: return null
    }
  }

  const NavItem = ({ t }) => (
    <button onClick={() => setTab(t.id)} className={`nav-item relative ${tab === t.id ? 'active' : ''}`}>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex transition-colors">
      ...
      {/* IMPORT BANNER */}
      {importBanner && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-green-600 text-white text-sm px-5 py-2.5 rounded-xl shadow-lg">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 min-h-screen fixed left-0 top-0 bottom-0">
        ...
        <div className="px-5 pb-4">
          <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-4">
            ...
            <button onClick={switchToNewPerson} className="mt-3 text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
              ⇄ New person
            </button>
          </div>
        </div>
        ...
        <div className="px-5 py-3 border-t border-b border-gray-100 dark:border-gray-800">
          ...
          <div className="bg-gray-100 dark:bg-gray-800 rounded-full h-1.5 overflow-hidden">
            <div className="bg-gray-900 dark:bg-white h-full rounded-full" style={{width:`${Math.min(100,((learning.project||1)/300)*100).toFixed(1)}%`}} />
          </div>
        </div>
      ...
      <main className="flex-1 lg:ml-64 pb-20 lg:pb-0">
        ...
        <div className="lg:hidden flex items-center justify-between px-4 pt-6 pb-3">
          ...
          <div className="flex gap-2 items-center">
            <button onClick={toggleDark} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800">
              {dark ? '☀️' : '🌙'}
            </button>
            {urgentOpps > 0 && (
              <button onClick={() => setTab('opportunities')} className="bg-red-100 dark:bg-red-950 text-red-600 text-xs px-2.5 py-1.5 rounded-xl font-medium">⚠ {urgentOpps}</button>
            )}
          </div>
        </div>
